...
public class SensorRecord extends Activity
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
		...
		mWakeLock = mPm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK,"SensorRecord");
		mWakeLock.acquire();
...
//			mJNIBW.close();
	...
	class MyLocationListener implements LocationListener{
		...
		public void onLocationChanged(Location location){
			//Toast.makeText(SensorRecord.this,"Get Location",Toast.LENGTH_SHORT).show();
			mGpsSB.setLength(0);
			...
			//Toast.makeText(SensorRecord.this,mGpsSB.toString(),Toast.LENGTH_SHORT).show();
			++mGpsNum;
	...
	void scanJNI(){
		...
		mJNIThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				while(mIsscanJNI){
					try{
						mJNIBW = new BufferedWriter(new FileWriter(JNIFile,true));
